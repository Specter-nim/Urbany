...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def alerts_mock(request):
    """Endpoint mock de alertas con dos alertas estáticas (HU15)"""
    data = {
        'generated_at': timezone.now().isoformat(),
        'alerts': [
            {
                'id': 'mock-venta-vencida',
                'title': 'Recordatorio: Venta próxima a vencer',
                'message': 'La oferta para el cliente XYZ vence mañana.',
                'priority': 'high',
                'alert_type': 'reminder',
                'status': 'active',
                'scheduled_at': (timezone.now() + timedelta(hours=12)).isoformat()
            },
            {
                'id': 'mock-reunion',
                'title': 'Reunión con cliente',
                'message': 'Encuentro con María Pérez hoy a las 16:00.',
                'priority': 'medium',
